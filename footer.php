    ...
    if ($current_lang == 'dk') {
        $home_url = home_url('/dk/home-3/');
        $bieren_url = home_url('/dk/home-3/#onze-bieren-container');
    }
    else if ($current_lang == 'en') {
        $home_url = home_url('/en/home-2/');
        $bieren_url = home_url('/en/home-2/#onze-bieren-container');
    }
    else {
        $home_url = home_url('/home/');
        $bieren_url = home_url('/home/#onze-bieren-container');
    }

    $onzebieren_id = pll_get_post( 27 );
    $onzebieren_url = (get_permalink( $onzebieren_id ) . '#onze-bieren-container');

    $onsverhaal_id = pll_get_post( 210 );
    $onsverhaal_url = (get_permalink( $onsverhaal_id ));

    $waar_te_koop_id = pll_get_post( 124 );
    $waar_te_koop_url = (get_permalink( $waar_te_koop_id ));

    $contact_id = pll_get_post( 226 );
...
?>
    ...
    <footer class="footer">
            ...
            <div class="section footer__wrapper">
                ...
                <div>
                    <p class="caption"><?php if (function_exists('pll_e')) { pll_e('Ga snel naar'); } else { echo 'Ga snel naar'; } ?></p>
                    <div class="link-group">
                        <div>
                        <a href="<?php echo esc_url($onzebieren_url); ?> "class="link"><span><?php if (function_exists('pll_e')) { pll_e('Onze bieren'); } else { echo 'Onze bieren'; } ?></span><img src="<?php echo get_template_directory_uri(); ?>/assets/arrow.svg"></a>
                        <a href="<?php echo esc_url($onsverhaal_url); ?>" class="link"><span><?php if (function_exists('pll_e')) { pll_e('Ons verhaal'); } else { echo 'Ons verhaal'; } ?></span><img src="<?php echo get_template_directory_uri(); ?>/assets/arrow.svg"></a>
                        </div>
                        <div>
                        <a href="<?php echo esc_url($waar_te_koop_url); ?>" class="link"><span><?php if (function_exists('pll_e')) { pll_e('Waar te koop?'); } else { echo 'Waar te koop?'; } ?></span><img src="<?php echo get_template_directory_uri(); ?>/assets/arrow.svg"></a>
                        <a href="<?php echo esc_url($contact_url); ?>" class="link"><span><?php if (function_exists('pll_e')) { pll_e('Contact'); } else { echo 'Contact'; } ?></span><img src="<?php echo get_template_directory_uri(); ?>/assets/arrow.svg"></a>    
                        </div>
                    </div>
                </div>
                <div>
                    <p class="caption"><?php if (function_exists('pll_e')) { pll_e('Adres'); } else { echo 'Adres'; } ?></p>
                    <p class="link">Brouwgilde Hulsbergen<br>Kloosterweg 3<br>8191 JA Wapenveld<br><?php if (function_exists('pll_e')) { pll_e('Nederland'); } else { echo 'Nederland'; } ?></p>
                <a class="link" href="mailto:info@brouwgildehulsbergen.nl">
            ...
            <div class="disclaimer">
                    <a class="btn" href="https://www.brouwgildehulsbergen.nl/" target="_blank">
            <img class="logo--hulsbergen" src="<?php echo get_template_directory_uri(); ?>/assets/hulsbergen_logo.png">
            </a>
            <div class="info-group">
                <div class="btn-group">
                    <a><p><?php if (function_exists('pll_e')) { pll_e('Disclaimer'); } else { echo 'Disclaimer'; } ?></p></a>
                </div>
                    <div class="icon--large spacing--water">
                        <svg viewBox="0 0 48 21" style= "fill: #F6F3F0;">
                ...
                <div class="btn-group">
                    <a><p><?php if (function_exists('pll_e')) { pll_e('Auteursrechten'); } else { echo 'Auteursrechten'; } ?></p></a>
                </div>
            </div>
            <div class="btn-group">
            <p><?php if (function_exists('pll_e')) { pll_e('Ontwerp door:'); } else { echo 'Ontwerp door:'; } ?> <a href="https://www.wouterhof.com" target="_blank">wouterhof.com</a></p>
            </div>
            </div>
    </footer>
    <?php wp_footer(); ?>
